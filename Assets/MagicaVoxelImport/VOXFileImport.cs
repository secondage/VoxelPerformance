...
namespace Cubizer
{
	namespace Model
	{
		...
		public class VoxFileImport
		{
            ...
            public static GameObject CreateVoxelGrid(string name, VoxData data, Texture2D texture, Color32[] colors, float scale)
            {
                var cruncher = VOXPolygonCruncher.CalcVoxelCruncher(data, colors, VOXCruncherMode.Greedy);
            ...
            public static GameObject LoadVoxelFileAsVoxelGrid(string name, VoxFileData voxel, int lodLevel)
            {
                Debug.Assert(!String.IsNullOrEmpty(name));
                ...
                try
                {
                    ...
                    if (lodLevel <= 1)
                    {
                        foreach (var chunk in voxel.chunkChild)
                        {
                            var submesh = CreateVoxelGrid("model", chunk.xyzi.voxels, texture, colors, 1);
                            submesh.transform.parent = gameObject.transform;
                        }
                    }
                    else
            ...
            public static GameObject LoadVoxelFileAsVoxelGrid(string path)
            {
                var voxel = VoxFileImport.Load(path);
                return LoadVoxelFileAsVoxelGrid(Path.GetFileNameWithoutExtension(path), voxel, 0);
            }
